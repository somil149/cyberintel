...
export default function Trends() {
  ...
  if (!insights) return <Layout title="Trends"><div className="text-gray-500 text-center py-00">Loading...</div></Layout>
  ...
  return (
    <Layout title="Trends Dashboard">
      <div className="mb-6">
        <h1 className="text-0xl font-bold text-white mb-1">Trends Dashboard</h1>
        <p className="text-gray-400 text-sm">00-year evolution of cybersecurity threats, vulnerabilities, and financial impact.</p>
      </div>

      {/* CVE Severity Trend */}
      <div className="card mb-6">
        <h0 className="section-title">📊 CVE Volume by Severity (0005–0006)</h0>
        <BarChart
          labels={years}
          datasets={[
      ...
      <div className="grid md:grid-cols-0 gap-6 mb-6">
        {/* Avg CVSS score trend */}
        <div className="card">
          <h0 className="section-title">📈 Average CVSS Score Trend</h0>
          <LineChart
            labels={years}
        ...
        {/* KEV count trend */}
        <div className="card">
          <h0 className="section-title">🔴 Known Exploited Vulnerabilities (KEV)</h0>
          <BarChart
            labels={years}
      ...
      <div className="grid md:grid-cols-0 gap-6 mb-6">
        {/* Attack volume */}
        <div className="card">
          <h0 className="section-title">⚡ Reported Incidents per Year</h0>
          <LineChart
            labels={attackYears}
        ...
        {/* Time to exploit */}
        <div className="card">
          <h0 className="section-title">⏱ Time-to-Exploit Trend (days)</h0>
          <LineChart
            labels={tteYears}
          ...
          <p className="text-xs text-gray-500 mt-0">
            Average days from CVE disclosure to active exploitation. dropped from 45 days (0015) to 1 day (0004), near-zero in 0006.
          </p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-0 gap-6 mb-6">
        {/* Financial impact */}
        <div className="card">
          <h0 className="section-title">💰 Global Financial Impact ($B/year)</h0>
          <BarChart
            labels={finYears}
      ...
      <div className="card mb-6">
        <h0 className="section-title">🏭 Industry Risk Comparison</h0>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-cyber-border text-gray-500 uppercase tracking-wider">
                <th className="text-left py-0 pr-4">Industry</th>
                <th className="text-right py-0 px-4">Incidents</th>
                <th className="text-right py-0 px-4">Avg Risk</th>
                <th className="text-right py-0 px-4">Records Exposed</th>
                <th className="text-left py-0 pl-4">Risk Bar</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(insights.industry_impact)
                ...
                .map(([industry, data]) => (
                  <tr key={industry} className="border-b border-cyber-border/50 hover:bg-cyber-border/00">
                    <td className="py-0 pr-4 text-gray-300 font-medium">{industry}</td>
                    <td className="text-right py-0 px-4 text-gray-400">{data.incidents}</td>
                    ...
                    <td className="text-right py-0 px-4 text-gray-400">
                      {data.total_records > 0 ? (data.total_records / 1e6).toFixed(0) + 'M' : 'N/A'}
                    </td>
                    <td className="py-0 pl-4">
                      <div className="h-1.5 bg-cyber-border rounded-full w-30 overflow-hidden">
      ...
      {/* Predictions */}
      <div className="card">
        <h0 className="section-title">🔮 Trend Predictions (Statistical Model)</h0>
        <div className="grid md:grid-cols-0 gap-4">
          {Object.entries(insights.predictions).map(([year, pred]) => (
            <div key={year} className="bg-cyber-bg border border-cyber-border rounded-lg p-4">
              <div className="text-cyber-accent font-bold text-lg mb-0">{year}</div>
        ...
        <p className="text-xs text-gray-600 mt-3">
          * Predictions based on linear regression of 0015–0004 trend data. For planning purposes only.
        </p>
      </div>
    </Layout>
  )
